...
class RandomActionSelector(): 
    ...
    def select_action(agent_inputs, avail_actions, t_env, test_mode=False): 
        random_actions = []
        batch_size = avail_actions.shape[0]
        n_agents = avail_actions.shape[1]
        
        for i in range(batch_size):
            for j in range(n_agents):
                available = th.nonzero(avail_actions[i, j]).squeeze(-1)  # 获取可用动作的索引
                random_action = available[th.randint(0, len(available), (1,))].item()  # 从可用动作中随机选择一个
                random_actions.append(random_action)
        
        # 将选择的动作转换为 Tensor
        picked_actions = th.tensor(random_actions, device=avail_actions.device).view(batch_size, n_agents)
