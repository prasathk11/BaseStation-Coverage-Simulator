...
gru_model = GRU()
gru_model.load_state_dict(torch.load('/Users/hazelyu/Library/Mobile Documents/com~apple~CloudDocs/Documents/2024Spring/gru/gru/gru_12u_norm.pt'))
...
class DT_MARL(MultiAgentEnv):
    ...
    def get_avail_agent_actions(self, agent_id):
        ...
        for action in range(self.n_actions): 
            action_bits = format(action, "013b")
    ...
    def step(self, actions): # 必须实现的函数，输入动作，输出下一步的状态、奖励、结束与否、其他可选info
        ...
        actions_vec = []
        actions_str = [format(action, "013b") for action in actions]
