...
GRU_MODEL_PATH = PROJECT_ROOT / "gru_10u_norm.pt"
gru_path_str = str(GRU_MODEL_PATH)
...
gru_model = GRU()
gru_model.load_state_dict(torch.load(gru_path_str))
...
class Setting():
    def __init__(self):
        ...
        self.RbInfo = [0.017, 0.021, 0.025]   # 按需自定义设置rb的相关信息，这里是I_n
...
class DT_MARL_10u_org(MultiAgentEnv):
    ...
    def step(self, actions): # 必须实现的函数，输入动作，输出下一步的状态、奖励、结束与否、其他可选info
       ...
       # print(self.setting.UserLocation, self.usr_dist)
        ...
        self.stepInfo = {
            #   'user_position': self.get_state(), # 当前PN的state
            ...
            #   'Ans_mat': ans_mat
            # "DNTaccuracy": acc, 
            # "dataratemean": np.sum(data_rate) / self.setting.num_ue
        }
