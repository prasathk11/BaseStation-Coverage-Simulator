...
gru_model = GRU()
gru_model.load_state_dict(torch.load('/Users/hazelyu/Library/Mobile Documents/com~apple~CloudDocs/Documents/2024Spring/gru/gru/gru_10u_norm.pt'))
...
class Setting():
    def __init__(self):
        self.BS_loc = np.array([[-160,0], [0,0], [160,0]])
        self.num_BS = len(self.BS_loc)
        self.BS_coverage = 150
        self.usr_mov_prob = [
            (0.85, 0, 0, 0.15), 
            (0.85, 0, 0.15, 0), 
            (0, 0, 1, 0),   
        ...
        self.CloudSeverLocation = np.array([0,150],dtype=np.float32)
...
# Environment class
class DT_MARL_10u_org(MultiAgentEnv):
    ...
    def step(self, actions): # 必须实现的函数，输入动作，输出下一步的状态、奖励、结束与否、其他可选info
        ...
        for agent_id in range(self.n_agents): 
            if actions_vec[agent_id][-1] == 0: 
                ...
                self.reward[agent_id] -= 0.75 * np.mean((np.array(dt_obs) - np.array(obs)) ** 2) 
